                ...
                <div className=" px-4 py-8 flex flex-col gap-3 w-full">
                    {titles.map((title, index) => (
                        <a
                            href={`#${title}`}
                            key={index}
                            className={`ml-2 opacity-70 transition-transform transform 
         ${activeTitle === title ? "scale-105 opacity-100 font-semibold" : ""
                                }
            ...
            {/* Button */}
            <button onClick={toggleTOC} className='hover:scale-105 transition-transform duration-200 shadow-md rounded-full backdrop-blur-2xl bg-kopken-primary-400/70 dark:bg-kopken-primary-500/50 p-3 fixed bottom-6 right-6 z-50 w-["88px"] h-["88px"]'>
